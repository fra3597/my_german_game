...
class ArticleController(Controller):
    def __init__(self, ui_controller):
        ...
        self.ui_controller.stacked_widget.addWidget(self.article_mode_window)
        self.ui_controller.stacked_widget.addWidget(self.article_summary_window)
        self.ui_controller.stacked_widget.addWidget(self.database_window)

        self.connect_buttons()

    def set_article_mode_window(self):
        self.ui_controller.stacked_widget.setCurrentWidget(self.article_mode_window)

    def start_game(self):
        self.set_article_mode_window()
        ...
        self.article_mode_window.widget.title_label.setText(text_to_show)

    def check_answer(self):
        user_answer = self.article_mode_window.read_user_answer()
        ...
        if user_answer == self.game_model.list_of_words[self.game_model.current_word].article:
            ...
            self.article_mode_window.widget.comparison_result_label.setText(result)
            self.game_model.score.update_partial_score()
            self.article_summary_window.append_row_to_summary_table(
                user_answer,
        ...
        if self.game_model.current_word == self.game_model.QUESTIONS_PER_GAME:
            self.article_mode_window.enable_go_to_summary_button()
    ...
    def connect_buttons(self):
        ...
        self.article_mode_window.connect_der_button(self.check_answer)
        self.article_mode_window.connect_die_button(self.check_answer)
        self.article_mode_window.connect_das_button(self.check_answer)
        self.article_mode_window.connect_go_to_summary_button(self.show_summary)
        self.article_mode_window.connect_open_database_button(self.open_database)

        self.article_summary_window.connect_play_again_button(self.play_again)
        self.article_summary_window.connect_quit_button(self.ui_controller.quit_game)
